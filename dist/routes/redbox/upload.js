...
async function convertPdfToImages(buffer) {
    try {
        ...
        const { writeFileSync, readFileSync, unlinkSync, existsSync } = await Promise.resolve().then(() => __importStar(require('fs')));
...
const router_instance = new hono_1.Hono();
router_instance.post("/upload", async (c) => {
    ...
    if (!allowedTypes.includes(file.type)) {
        return c.json({ error: "Unsupported file type. Upload PDF, Word doc, or image." }, 415);
    }
    ...
    if (file.type === "application/pdf") {
        ...
        if (textContent.length < 20) {
            ...
            if (imageContents.length === 0) {
                return c.json({
                    error: "Could not process this PDF. Please try uploading as an image.",
                }, 422);
            }
    ...
    try {
        ...
        if (extractionMode === "text") {
            ...
            completion = await openai_1.openai.chat.completions.create({
                ...
                messages: [
                    { role: "system", content: openai_1.EXTRACTION_SYSTEM_PROMPT },
                    {
                        role: "user",
    ...
    if (dbError || !submission) {
        ...
        return c.json({ error: "Failed to save submission", detail: dbError?.message }, 500);
    }
    return c.json({ submissionId: submission.id, extraction });
});
exports.default = router_instance;
